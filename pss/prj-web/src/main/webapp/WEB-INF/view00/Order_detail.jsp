...
<%--	<img class="avatar size-XL l" src="/js/1.jpg">--%>
	<dl style="margin-left:80px; color:#fff">
	</dl>
</div>
<div class="pd-20">
	<article class="page-container">
		<input type="hidden" name="userId" value="${bean.orderId}" /> <div class="row cl">
		<label class="form-label col-xs-4 col-sm-3">ID:</label>
		<div class="formControls col-xs-8 col-sm-9">${bean.orderId}</div> </div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">客户名称:</label>
			<div class="formControls col-xs-8 col-sm-9">${bean.cusName}</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">员工名称:</label>
			<div class="formControls col-xs-8 col-sm-9">${bean.cName}</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">发送地址:</label>
			<div class="formControls col-xs-8 col-sm-9"> ${bean.sentAdd}</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">发票号:</label>
			<div class="formControls col-xs-8 col-sm-9"> ${bean.invNum}</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">总交易金额:</label>
			<div class="formControls col-xs-8 col-sm-9"> ${bean.money}</div>
		</div>

		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">交易日期:</label>
			<div class="formControls col-xs-8 col-sm-9"> ${bean.orderDate}</div>
		</div>
