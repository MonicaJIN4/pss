...
<nav class="breadcrumb">
    ...
    商品管理 <span class="c-gray en">&gt;</span> 供应商
...
<div class="page-container">
    ...
    <form action="Supplier" method="get">
        ...
        <input type="text" class="input-text" value="${searchName}"
               style="width:250px;" placeholder="输入名称" id="searchName" name="searchName" >
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="javascript:;"
               onclick="item_add('添加','Supplier?oper=insert','800','500')"
    ...
    <div class="mt-20">
        <table id="datalist" class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <c:forEach var="item" items="${DataList }">
                <tr class="text-c">
                    <td><input type="checkbox" value="${item.supId}" name=""></td>
                    <td>${item.supId }</td>
                    <td>${item.name }</td>
                    <td>${item.tel }</td>
                    <td>${item.address }</td>
                    <%--<td class="td-status">--%>
                        <%--<c:choose>--%>
                            <%--&lt;%&ndash;<c:when test="${item.isDeleted=='1'}">&ndash;%&gt;--%>
                                <%--&lt;%&ndash;<span class="label label-success radius">是</span>&ndash;%&gt;--%>
                            <%--&lt;%&ndash;</c:when>&ndash;%&gt;--%>
                    ...
                    <td>
                        <a title="删除" href="javascript:;"
                           onclick="item_del(this,${item.supId})" class="ml-5"
                           style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        <a title="编辑" href="javascript:;"
                           onclick="item_edit('编辑[id=${item.supId}]','Supplier?oper=update&id=${item.supId }','1','800','500')"
                           class="ml-5" style="text-decoration: none">
                            <i class="Hui-iconfont">&#xe6df;</i></a>
                        <%--<a title="查看" href="javascript:;"--%>
                           <%--onclick="item_detail('查看[id=${item.supId}]','Supplier?oper=detail&id=${item.supId }','4','800','500')"--%>
                           <%--class="ml-5" style="text-decoration: none">--%>
                            <%--<i class="Hui-iconfont">&#xe707;</i></a>--%>
                    </td> </tr>
            </c:forEach>
...
<script type="text/javascript">
    $(function(){
        $("#clearSearch").click(function(){
            location.href = "Supplier?oper=list";
        });
    ...
    /*项目—查看*/
    function  item_detail(tile,url,id,w,h) {
        layer_show(tile,url,w,h);
    ...
    function item_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: 'Supplier?oper=deleteDeal&&id=' + id,
                //dataType: 'json',
                success: function(data){
                    if(data=="ok"){
