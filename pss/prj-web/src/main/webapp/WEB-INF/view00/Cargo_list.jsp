...
<nav class="breadcrumb">
	...
	商品管理 <span class="c-gray en">&gt;</span> 商品资料
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			<a href="javascript:;"
			   onclick="item_add('添加','Cargo?oper=insert','800','500')"
			   class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加
			</a>
			<button class="btn btn-primary" style="border-radius: 5px">
				<a href="Cargo?oper=excel" download="商品资料表.xlsx" style="color: #ffffff">导出数据</a>
			</button>
			</span>
	...
	<div class="mt-20">
		<table id="datalist" class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<c:forEach var="item" items="${DataList }"> <tr class="text-c">
				<td><input type="checkbox" value="${item.cargoId }" name=""></td>
				<td>${item.cargoId }</td>
				...
				<td>
					...
					<a title="编辑" href="javascript:;"
					   onclick="item_edit('编辑[id=${item.cargoId}]','Cargo?oper=update&id=${item.cargoId}','1','800','500')"
					   class="ml-5" style="text-decoration: none">
						<i class="Hui-iconfont">&#xe6df;</i></a>
						<%--<a title="查看"--%>
						<%--href="javascript:;"--%>
						<%--onclick="item_detail('查看[Id=${item.cargoId}]','Cargo?oper=detail&id=${item.cargoId }','4','800','500')"--%>
						<%--class="ml-5" style="text-decoration: none">--%>
						<%--<i class="Hui-iconfont">&#xe707;</i></a>--%>
				</td> </tr>
			</c:forEach>
...
<script type="text/javascript">
	...
	function item_del(obj,id){
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				url: 'Cargo?oper=deleteDeal&&id=' + id,
				//dataType: 'json',
				success: function(data){
					if(data=="ok"){
