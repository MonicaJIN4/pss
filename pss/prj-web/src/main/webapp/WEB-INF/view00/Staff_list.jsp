...
<nav class="breadcrumb">
    ...
    销售管理 <span class="c-gray en">&gt;</span>销售员工资料
...
<div class="page-container">
    <div class="text-c">
        <form action="Staff" method="get">
            ...
            <input type="text" class="input-text" value="${searchName}"
                   style="width:250px;" placeholder="输入中文名" id="searchName" name="searchName" >
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="javascript:;" onclick="item_add('添加','Staff?oper=insert','800','500')"
    ...
    <div class="mt-20">
        <table id="datalist" class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <c:forEach var="item" items="${DataList}">
                <tr class="text-c">
                    <td><input type="checkbox" value="${item.staffId}" name=""></td>
                    <td>${item.staffId}</td>
                    <td>${item.cName}</td>
                    <td>${item.mPhone}</td>
                    <td>${item.email}</td>
                    <td>
                        <a title="删除" href="javascript:;" onclick="item_del(this,${item.staffId})" class="ml-5" style="text-decoration: none">
                            <i class="Hui-iconfont">&#xe6e2;</i>
                        </a>
                        <a title="编辑" href="javascript:;" onclick="item_edit('编辑[id=${item.staffId}]',
                                'Staff?oper=update&id=${item.staffId}','1','800','500')" class="ml-5" style="text-decoration: none">
                            <i class="Hui-iconfont">&#xe6df;</i>
                        </a>
                        <a title="查看" href="javascript:;" onclick="item_detail('查看[Id=${item.staffId}]',
                                'Staff?oper=detail&id=${item.staffId }','4','800','500')" class="ml-5" style="text-decoration: none">
                            <i class="Hui-iconfont">&#xe707;</i>
                        </a>
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    function item_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: 'Staff?oper=deleteDeal&&id=' + id,
                //dataType: 'json',
