...
<nav class="breadcrumb">
	...
	客户管理 <span class="c-gray en">&gt;</span> 客户联系方式
...
<div class="page-container">
	<div class="text-c">
	<form action="CusContact" method="get">

		<input type="hidden" name="oper" value="listDeal" />

		<span>姓名：</span>
		<input type="text" class="input-text" value="${searchName}"
			style="width:250px;" placeholder="输入姓名" id="searchName" name="searchName" >
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="javascript:;"
			   onclick="item_add('添加','CusContact?oper=insert','800','500')"
...
<%--				<th width="80">是否删除</th>--%>
		...
		<tbody>
			<c:forEach var="item" items="${DataList }"> <tr class="text-c">
				<td><input type="checkbox" value="${item.cusId }" name=""></td>
				<td>${item.cusId }</td>
				<td>${item.cusName }</td>
				<td>${item.cusTel }</td>
<%--				<td class="td-status">--%>
...
<%--				</td>--%>
				<td>
					<a title="删除" href="javascript:;"
					   onclick="item_del(this,${item.cusId})" class="ml-5"
					   style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					<a title="编辑" href="javascript:;"
					   onclick="item_edit('编辑[id=${item.cusId}]','CusContact?oper=update&id=${item.cusId}','1','800','500')"
						class="ml-5" style="text-decoration: none">
						<i class="Hui-iconfont">&#xe6df;</i></a>
					<a title="查看"
					   href="javascript:;"
					   onclick="item_detail('查看[Id=${item.cusId}]','CusContact?oper=detail&id=${item.cusId }','4','800','500')"
					   class="ml-5" style="text-decoration: none">
						<i class="Hui-iconfont">&#xe707;</i></a>
				</td> </tr>
			</c:forEach>
...
$(function(){
	$("#clearSearch").click(function () {
		location.href="CusContact?oper=list";
...
function item_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'CusContact?oper=deleteDeal&&id=' + id,
			//dataType: 'json',
			success: function(data){
				if(data=="ok"){
