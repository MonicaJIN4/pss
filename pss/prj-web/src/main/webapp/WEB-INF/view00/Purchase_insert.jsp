...
<article class="page-container">
	<form action="Purchase"  method="post" class="form form-horizontal" id="objForm">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select id="cargoId" name="cargoId" class="select-box" style="width: 200px">
					...
					<c:forEach var="item" items="${cargoList}">
						<option value="${item.cargoId}" onclick="fun(${item.buyPrice},'${item.unit}')"> ${item.cargoName}</option>
					</c:forEach>
				</select>
				<button class="btn radius btn-primary size-S" onclick="item_add('添加商品','Cargo?oper=insert','600','400')">添加商品</button>
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${buyPrice}" style="width: 200px;" placeholder="" id="buyPrice" name="buyPrice">
				元
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span id="adviceBuyPrice" style="font-weight: bold;">
				</span>&nbsp;元/<span name="unit"></span>
			</div>
		</div>
		<div class="row cl">
