...
<nav class="breadcrumb">
	...
	客户管理 <span class="c-gray en">&gt;</span> 客户资料
...
<div class="page-container">
	<div class="text-c">
	<form action="CusOper" method="get">

		<input type="hidden" name="oper" value="listDeal" />

		<span>公司名称：</span>
		<input type="text" class="input-text" value="${searchName}"
			style="width:250px;" placeholder="输入公司名称" id="searchName" name="searchName" >
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="javascript:;"
			   onclick="item_add('添加','CusOper?oper=insert','800','500')"
	...
	<table id="datalist" class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach var="item" items="${DataList }"> <tr class="text-c">
				<td><input type="checkbox" value="${item.operId }" name=""></td>
				<td>${item.operId }</td>
				<td>${item.cusName }</td>
				<td>${item.cName }</td>
				<td>${item.cusTel }</td>
				...
				<td>
					<a title="删除" href="javascript:;"
					   onclick="item_del(this,${item.operId})" class="ml-5"
					   style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					<a title="编辑" href="javascript:;"
					   onclick="item_edit('编辑[id=${item.operId}]','CusOper?oper=update&id=${item.operId}','1','800','500')"
						class="ml-5" style="text-decoration: none">
						<i class="Hui-iconfont">&#xe6df;</i></a>
					<a title="查看"
					   href="javascript:;"
					   onclick="item_detail('查看[Id=${item.operId}]','CusOper?oper=detail&id=${item.operId }','4','800','500')"
					   class="ml-5" style="text-decoration: none">
						<i class="Hui-iconfont">&#xe707;</i></a>
				</td> </tr>
			</c:forEach>
...
$(function(){
	$("#clearSearch").click(function () {
		location.href="CusOper?oper=list";
...
function item_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'CusOper?oper=deleteDeal&&id=' + id,
			//dataType: 'json',
			success: function(data){
				if(data=="ok"){
