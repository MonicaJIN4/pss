...
@WebServlet(UIConst.AREAPATH + "/CusContact")
public class CusContactServlet extends HttpServlet {
    private CusContactService cusContactService = new CusContactServiceImpl();
...
//        }
    ...
    protected void listView(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        List<CusContact> vDataList =null;
        ...
        rowCount = cusContactService.count();
        // 分页步骤2.3. 将记录数赋给pagerItem，以便进行分页的各类计算
        pagerItem.changeRowCount(rowCount);

        // 分页步骤2.4. 从数据库取指定分页的数据 ***** pager()要根据实际换成其它方法
        vDataList =  cusContactService.pager(pagerItem.getPageNum(), pagerItem.getPageSize());
        // 分页步骤2.5. 设置页面的跳转url
        ...
        // 转发到页面
        String toPage = UIConst.VIEWPATH + "/CusContact_list.jsp";
    ...
    protected void listDeal(HttpServletRequest request, HttpServletResponse response)
        ...
        if (SysFun.isNullOrEmpty(searchName)) {
            ...
            // 分页步骤2.2. 根据条件，查找符合条件的所有记录数 ***** count()要根 据实际换成其它方法
            rowCount = cusContactService.countByName(searchName);
            // 分页步骤2.3. 将记录数赋给pagerItem，以便进行分页的各类计算      
            pagerItem.changeRowCount(rowCount);
            // 分页步骤2.4. 从数据库取指定分页的数据 ***** pager()要根据实际换成 其它方法
            vDataList = cusContactService.pagerByName(searchName, pagerItem.getPageNum(), pagerItem.getPageSize());
        }
        // 分页步骤2.5. 设置页面的跳转url
        pagerItem.changeUrl(SysFun.generalUrl(request.getRequestURI(),request.getQueryString()));
        // 分页步骤3. 将分页对象和数据列表,放到作用域,以便页面可以访问
    ...
    protected void insertView(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String toPage = UIConst.VIEWPATH + "/CusContact_insert.jsp";
        request.getRequestDispatcher(toPage).forward(request,response);
    ...
    protected void insertDeal(HttpServletRequest request, HttpServletResponse response)
        ...
        //服务端验证
        String vMsg = "";
        if(SysFun.isNullOrEmpty(cusName))
        {
            vMsg+= "客户名不能为空";
        }
        ...
        Long result  = 0L;
        try {
            result=cusContactService.insert(bean);
        }catch(Exception e){
    ...
    protected void updateView(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!SysFun.isNullOrEmpty(vId)){
            ...
            CusContact bean = cusContactService.load(iId);
            ...
            if (bean != null){
                ...
                request.setAttribute("cusName",bean.getCusName());
                request.setAttribute("cusTel",bean.getCusTel());
                String toPage =UIConst.VIEWPATH+ "/CusContact_update.jsp";
                request.getRequestDispatcher(toPage).forward(request,response);
                return;
            }
        }
    ...
    protected void updateDeal(HttpServletRequest request, HttpServletResponse response)
        ...
        java.io.PrintWriter out = response.getWriter();
        // (0) 获取请求数据
        Integer vId = Integer.valueOf(request.getParameter("cusId"));
        String cusName = request.getParameter("cusName");
        String cusTel = request.getParameter("cusTel");
        // 为了在输入页面回显原来的旧值,需要将旧值放到作用域,页面中进行获取
        request.setAttribute("cusId", vId);
        request.setAttribute("cusName",cusName);
        request.setAttribute("cusTel",cusTel);
        ...
        // (3) 真正处理
        bean.setCusId(Math.toIntExact(iId));
        bean.setCusName(cusName);
        bean.setCusTel(cusTel);

        Long result = 0L;
        try {
            result = cusContactService.update(bean);
        } catch (Exception e) {
    ...
    protected void detailView(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!SysFun.isNullOrEmpty(vId)){
            ...
            CusContact bean = cusContactService.load(iPK);
            if (bean != null) {
                // 使用对象来回显
                request.setAttribute("bean",bean);
                String toPage = UIConst.VIEWPATH + "/CusContact_detail.jsp";
                request.getRequestDispatcher(toPage).forward(request,response);
                return;
            }
        }
    ...
    protected void deleteDeal(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!SysFun.isNullOrEmpty(vId)){
            ...
            Long result = 0L;
            result = cusContactService.delete(iId);
